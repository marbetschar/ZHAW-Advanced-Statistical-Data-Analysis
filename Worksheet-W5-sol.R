...
## mu_i = c'(mu)/b'(mu) = 1/(1/mu) = mu
## V(mu) = 1/b'(mu) = 1/(1/mu) = mu
## var(Y_i) = phi/w_i * V(mu) = 1/1 * mu = mu
...
par(mfrow=c(2,4))
for (k in 1:(ncol(DaR)-1)) hist(DaR[,k], col="gray",
                            main=paste("Histogram of", names(DaR)[k]))
which((DaR$AR > 200) | (DaR$RDR > 1500)) ## 1 33 35 40 53



## (b)
## Least squares fit on the untransformed data using all data
...
## There is CLEAR evidence that the expectation is not constant (probably
## cuased by the outliers), the residuals are not Gaussian distributed  and
## that the variance of the residuals is not constant because the "structures"
## are outside the stochastic fluctuation.
## There is no too influential observation.
...
DaR$sVH <- sqrt(DaR$VH)
DaR$sPOP <- sqrt(DaR$POP)
DaR$sRDR <- sqrt(DaR$RDR)
## and AR by logarithm
DaR$lAR <- log(DaR$AR)

## Now, check with a additive model:
library(gam)
DaR.gam1 <- gam(sRDR ~ lo(sPOP) + lo(lAR) + lo(HR) + lo(sVH) + lo(F) + fvIND,
                data=DaR, bf.maxit=100)
par(mfrow=c(2,3))
plot(DaR.gam1, se=TRUE)

## A bit more smoothing:
DaR.gam2 <- gam(sRDR ~ lo(sPOP, span=0.7) + lo(lAR, span=0.7) + lo(HR, span=0.7)
                + lo(sVH, span=0.7) + lo(F, span=0.7) + fvIND,
                data=DaR, bf.maxit=100)
par(mfrow=c(2,3))
plot(DaR.gam2, se=TRUE)

DaR[DaR$HR > 22,]  ## these obs may be "bad" leverage points

## Let's try
DaR$tF <- ifelse(DaR$F < 0.3, DaR$F, 0.3)

DaR.gam3 <- gam(sRDR ~ lo(sPOP, span=0.7) + lo(lAR, span=0.7) + lo(HR, span=0.7)
                + lo(sVH, span=0.7) + lo(tF, span=0.9) + fvIND,
                data=DaR, bf.maxit=100)
par(mfrow=c(2,4))
plot(DaR.gam3, se=TRUE)


## Robust Fitting:
library(robustbase)
DaR.rlm <- lmrob(sRDR ~ sPOP + lAR + HR + sVH + tF + fvIND, data=DaR)
summary(DaR.rlm)
## Robustness weights:
...
## Again by LS:
DaR1.lm1 <- lm(sRDR ~ sPOP + lAR + HR + sVH + tF + fvIND, data=DaR1)

par(mfrow=c(2,4))
plot(DaR1.lm1)
plot.lmSim(DaR1.lm1, SEED=147)
## There is no evidence that the expectation is not constant, the residuals
## are not Gaussian distributed  and that the variance of the residuals is
## not constant because the "structures" are inside the stochastic fluctuation.
## None of the observations is too influential since there Cook's distance is
## smaller than 1.
##

summary(DaR1.lm1)
## We need all variables ro explain the response.

## We still need to clarify why there are three outliers.
...
## the explanatory variables lPOP, lAR, lHR, lVH, lF, and IND.
...
## Question 3
## ~~~~~~~~~~
Bak <- read.table("Data/bacteria.dat", header=T, stringsAsFactors=TRUE)
str(Bak)
...
##
## (c)
confint.default(Bak.glm,2)
##           2.5 %     97.5 %
## Time -0.2334503 -0.2043898

## by hand using readings of the summary output:
-0.21892 + c(-1,1)*1.96*0.007413474  ## = [-0.2334504, -0.2043896]
## by hand using the summary output:
...
## The values do almost agree. Is this just a question of numerical inaccuracy
...
## Question 4
## ~~~~~~~~~~
transa <- read.table("Data/transactions.dat", header=T, stringsAsFactors=TRUE)
str(transa)
...
## Question 5
## ~~~~~~~~~~
nw <- read.table("Data/nambeware.txt", header=T, stringsAsFactors = TRUE)
summary(nw)
...
## Thus the estimated expected response is affected by Diam and a factor exp(beta2)
