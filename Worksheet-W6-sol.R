...
par(mfrow=c(2,3))
for(k in 2:6){
    sunflowerplot(baby[,k], baby$Survival, ylim=c(-.1,1.1))
    title(main=names(baby)[k])
    }
...
## Apgar5       0.0611499  0.1202222   01.389   0.6972
## ---
...
## No, just Weight and Age are significant on the 5% level.
## Apgar1, Apgar5 and pH are not significant at the 5% level because the
## corresponding p-value is larger than 5%. Consequently, at least one variable
...
## As suggested:
tm.glmA <- glm(Failures ~ -1 + Mode1, family=poisson(link="identity"), data=tm)
summary(tm.glmA) ##
