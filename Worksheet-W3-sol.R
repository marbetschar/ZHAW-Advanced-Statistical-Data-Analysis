...
library(gam)
PP1.gam <- gam(prestige ~ lo(income) + lo(education) + lo(women) + type,
               data=PP1)
par(mfrow=c(2,2))
plot(PP1.gam, se=TRUE)
summary(PP1.gam)

## transformations are required for income (and woman) becaus a straight line
## does not fit between the pointwise confidence intervals
## I suggest the following transformations:
PP1$lIncome <- log(PP1$income)
PP1$tWomen <- ifelse(PP1$women > 40, PP1$women, 40)

## are further transformations required?
PP1.gam2 <- gam(prestige ~ lo(lIncome) + lo(education) + lo(tWomen)  + type,
                data=PP1)
par(mfrow=c(2,2))
plot(PP1.gam2, se=TRUE)
summary(PP1.gam2)
## According to the marginal tests under "Anova for Nonparametric Effects"
## no further transformations are required.

##
## (a-ii) Fit the corresponding linear model and run a residual analysis
PP1.lm1 <- lm(prestige ~ lIncome + education + tWomen + type, data=PP1)
summary(PP1.lm1)

source("RFn/RFn_Plot-lmSim.R")
par(mfrow=c(2,4))
plot(PP1.lm1)
plot.lmSim(PP1.lm1, SEED=492)
## - Residuals vs Fitted: Since the red smoother is mostly within the stochastic
##   fluctuation, we have no evidence that the expected error is not 0.
## - Q-Q Residuals: Since the black dots are within the stochastic
##   fluctuation, we have no evidence that the error distribution ist not
##   Gaussian.
## - Scale-Location: Since the red smoother is within the stochastic
##   fluctuation, we have no evidence that the variance of the error ist not
##   constant.
## - Since there ist no observation with a Cook's distance larger than 1,
##   there ist no too influencial observation.
## In summary, we have no evidence that any of the model asumption is violated.

##
## (b)
PP1.lm1a <- lm(prestige ~ lIncome + education + tWomen, data=PP1)
anova(PP1.lm1, PP1.lm1a)
## Since the P-value (= 0.0008442) is less than 5%, we can reject the null
## hypothesis that the three levels of type are equal in size, although the
## marginal test shows that both levels are not significant at the 5% level.
## That is, the 'prof' level is not significantly different from the 'bc' level,
## and the 'wc' level is not significantly different from the 'bc' level, but
## the 'wc' level is significantly different from the 'prof' level.
##


## (*)
## Alternative transformation for
PP1$tEduc <- ifelse(PP1$education > 9, PP1$education, 9)

PP1.gam3 <- gam(prestige ~ lo(lIncome) + lo(tEduc) + lo(tWomen)  + type,
                data=PP1)
par(mfrow=c(2,2))
plot(PP1.gam3, se=TRUE)
summary(PP1.gam3)
## According to the marginal tests under "Anova for Nonparametric Effects"
## lIncome should be further transformed!

##
## (a-ii) Fit the corresponding linear model and run a residual analysis
PP1.lm2 <- lm(prestige ~ lIncome + tEduc + tWomen + type, data=PP1)
summary(PP1.lm2)

source("RFn/RFn_Plot-lmSim.R")
par(mfrow=c(2,4))
plot(PP1.lm2)
plot.lmSim(PP1.lm2, SEED=492)
...
library(gam)  ## With gam from package mgcv see below
...
## confidence band, no transformation is needed for lDist, but one for
## lWeight. The plot suggests that we should not transform weight!

EDS.gam2 <- gam::gam(lCost ~ lo(weight) + lo(lDist), data=EDS)
par(mfrow=c(1,2))
plot.Gam(EDS.gam2, se=TRUE)
## this model fits adequately,
## that is, do not transform the predictor variable weight in this example

plot.Gam(EDS.gam2, se=TRUE, residuals=TRUE)
...
## on the left half and is just outside the stochastic fluctuation, so that
## there is some (slight) evidence that the variance is not constant.
...
par(mfrow=c(2,3))
plot(EDS.rlm1)
## Observation 9 is a very distinct outlier, but there is another less
## distinct outlier, obs. 13. They might have caused all the difficulties.

## Repeat the least-squares analysis without observation 9:
EDS.lm2 <- lm(lCost ~ weight + lDist, data=EDS[-c(9),])

## source("RFn/RFn_Plot-lmSim.R")
par(mfrow=c(2,4))
plot(EDS.lm2)
plot.lmSim(EDS.lm2, SEED=711)
...
## Is there a reason, based on the sampling scheme or data recording, why
## observation 9 should not be included?
...
## Exercise (2-a)
library(mgcv)
EDS.gam1 <- mgcv::gam(lCost ~ s(lWeight) + s(lDist), data=EDS)
EDS.gam2 <- mgcv::gam(lCost ~ s(weight) + s(distance), data=EDS)
...
##   which is outside the stochastic fluctuation, so there is  evidence that
##   the variance is not constant.
## The points of the normal Q-Q plot scatter within the stochastic fluctuation,
##   so there is no evidence that the error is not Gaussian distributed.
##   and there is no outlier!!
## Since the Cook's distance of all observations is smaller than 1, there are
##   NO too influential observations.

## Conclusion: The model does not adequately fit the data. The variability
##             of the residuals is not constant.


## We would not conclude that the model 'lCost ~ weight + sDist' is adequate
## with the gam function from the package gam:
EDS.gam3a <- gam::gam(lCost ~ lo(weight) + lo(sDist), data=EDS)
par(mfrow=c(1,2))
plot.Gam(EDS.gam3a, se=TRUE)
## According to this implementation of gam, the root transformation of Dist is
## not appropriate in the model "lCost ~ weight + sDist".

## This contradiction from the two implementations is unexpected and confusing.
## What could be the reason for this? - I suspect that the sample size of
## 20 observations is too small to safely apply such a flexible model as an
## additive model.
